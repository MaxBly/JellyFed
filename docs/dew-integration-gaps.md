# JellyFed — notes internes d'intégration client

> Document interne de suivi inter-projets. Il garde l'historique des contraintes observées
> avec un client web externe, mais JellyFed reste un plugin Jellyfin autonome.
>
> Mise à jour : A1 est désormais livré côté JellyFed via le layout aplati v2. Les adaptations
> éventuelles côté dew sont hors scope de ce dépôt.
...
- [x] **A1 🔴 Lecture des sources alternatives (transcodage)** — **livré côté JellyFed** : layout aplati v2 (1 dossier par item, fichiers de tous les peers dans le même dossier avec suffixe `[peer-X]`). La fusion multi-version repose sur Jellyfin, pas sur `FederationMediaSourceProvider`.
...
Avec l'aplatissement (A1), le `FederationMediaSourceProvider` et le sidecar `sources.json` sont supprimés : la fusion est assurée par MergeVersions natif JF, pas par un provider custom.
